module.exports = async function handler(req, res) {
  ...
  const { skill, task_prompt, agent_name, api_key, model_endpoint } = req.body;
  ...
  if (isAnthropic) {
    ...
    return res.status(200).json({ result: data.content[0].text, agent_name, skill });
  }
  ...
  data = await response.json();
  if (!response.ok) return res.status(500).json({ error: data.error?.message || 'API error' });
  return res.status(200).json({ result: data.choices[0].message.content, agent_name, skill });
}
