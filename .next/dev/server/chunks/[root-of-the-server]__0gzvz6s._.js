...
module.exports = async function handler(req, res) {
    ...
    const { rows: borRows } = await db.query('SELECT * FROM agents WHERE id = $1 AND deposit_paid = 1', [
        borrower_agent_id
    ]);
    ...
    const { rows: allRows } = await db.query('SELECT * FROM agents WHERE deposit_paid = 1 AND id != $1', [
        borrower_agent_id
    ]);
    ...
    if (Number(borrower.token_balance) >= borrow_tokens) {
        await db.query('UPDATE agents SET token_balance = token_balance - $1 WHERE id = $2', [
            borrow_tokens,
            borrower_agent_id
        ]);
...
//# sourceMappingURL=%5Broot-of-the-server%5D__0gzvz6s._.js.map