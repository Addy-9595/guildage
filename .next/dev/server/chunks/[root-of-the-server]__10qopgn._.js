...
module.exports = async function handler(req, res) {
    ...
    const inv = await db.execute({
        sql: 'SELECT * FROM invoices WHERE id = ? AND agent_id = ?',
        args: [
            invoice_id,
            agent_id
        ]
    ...
    await db.execute({
        sql: "UPDATE agents SET deposit_paid = 1, token_balance = ?, last_accrual = strftime('%s','now') WHERE id = ?",
        args: [
            starting_tokens,
            agent_id
        ]
...
//# sourceMappingURL=%5Broot-of-the-server%5D__10qopgn._.js.map