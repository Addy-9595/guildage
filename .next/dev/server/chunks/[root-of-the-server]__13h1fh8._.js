...
"[project]/Desktop/Guildage/pages/api/activity.js [api] (ecmascript)", ((__turbopack_context__, module, exports) => {

const { ensureReady } = __turbopack_context__.r("[project]/Desktop/Guildage/lib/db.js [api] (ecmascript)");
module.exports = async function handler(req, res) {
    if (req.method !== 'GET') return res.status(405).end();
    const db = await ensureReady();
    const swapsRes = await db.execute(`SELECT s.*, a1.name as requester_name, a1.owner_name as requester_owner, a2.name as provider_name, a2.owner_name as provider_owner FROM swaps s JOIN agents a1 ON s.requester_agent_id = a1.id JOIN agents a2 ON s.provider_agent_id = a2.id ORDER BY s.created_at DESC LIMIT 20`);
    const borrowsRes = await db.execute(`SELECT b.*, a1.name as borrower_name, a1.owner_name as borrower_owner, a2.name as provider_name, a2.owner_name as provider_owner FROM borrows b JOIN agents a1 ON b.borrower_agent_id = a1.id JOIN agents a2 ON b.provider_agent_id = a2.id ORDER BY b.created_at DESC LIMIT 20`);
    return res.status(200).json({
        swaps: swapsRes.rows,
        borrows: borrowsRes.rows
    });
};
}),
];

//# sourceMappingURL=%5Broot-of-the-server%5D__13h1fh8._.js.map